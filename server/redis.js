...
const serverConfigs = {
	SESSION_EXPIRE_TIME_IN_DAYS: 15,
	DB_CONNECTION_MAX_RETRY: 5,
	DB_CONNECTION_RETRY_WAIT_TIME_IN_SEC: 3,
};
...
class ClerkCache extends MemCache {
	async createSessionByClerkUserId(userId, sessionId) {
		try {
			if (!this.isReady()) {
				this.connect();
				return null;
	...
	async getSessionByClerkUserId(userId) {
		try {
			if (!this.isReady()) {
				this.connect();
				return null;
